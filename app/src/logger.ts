...
const workerTransport = new transports.Console({
  level: "debug",
  format: combine(label({ label: "WORKER" }), timestamp(), logFormat),
});
...
const agentTransport = new transports.DailyRotateFile({
  ...
  maxFiles: "14d",
  level: "info",
  format: combine(label({ label: "AGENT" }), timestamp(), logFormat),
});

const masterTransport = new transports.File({
  filename: "logs/master.log",
  level: "info",
  format: combine(label({ label: "MASTER" }), timestamp(), logFormat),
});
...
// 进程类型判断
export const logger = baseLogger.add(
  cluster.isPrimary
    ? masterTransport
    : process.env.TYPE === "AGENT"
      ? agentTransport
      : workerTransport,
);
